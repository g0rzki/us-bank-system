...
public class CardService(AppDbContext db, CardsGateway cardsGateway, ILogger<CardService> logger)
{
    ...
    public async Task<CardResponse> UpdateCardStatusAsync(Guid userId, Guid accountId, Guid cardId, UpdateCardStatusRequest request)
    {
        ...
        if (request.Status == CardStatus.Active && card.Status == CardStatus.Blocked
            && card.BlockedAt.HasValue
            && (DateTime.UtcNow - card.BlockedAt.Value).TotalHours < 24)
            throw new InvalidOperationException("Card cannot be unblocked within 24 hours of being blocked");
    ...
    private async Task SyncStatusFromGatewayAsync(Card card)
    {
        ...
        var mappedStatus = external.Status.ToUpperInvariant() switch
        {
            ...
            // REQUESTED/PRODUCING/SHIPPED — karta w lifecycle, nie zmieniamy naszego statusu
            "REQUESTED" or "PRODUCING" or "SHIPPED"       => card.Status,
            _                                             => card.Status
        };
